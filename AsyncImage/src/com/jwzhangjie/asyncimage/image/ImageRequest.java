...
import com.jwzhangjie.asyncimage.image.ImageLoader.ImageLoaderCallback;
...
public class ImageRequest {
     ...
     * 
     * @param context
     * @param loadMethod 自定义的图片加载方法,如果为null则用默认的加载方法
     */
    public void load(Context context, LoadMethod loadMethod) {
        if (mFuture == null) {
            if (sImageLoader == null)
                sImageLoader = ImageLoader.getInstance(context);
            mFuture = sImageLoader.loadImage(mUrl, new InnerCallback(), loadMethod,
                    mBitmapProcessor, mOptions, mCacheKey);
        }
    }
